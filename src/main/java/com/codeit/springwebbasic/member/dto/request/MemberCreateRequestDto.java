...
@Schema(description = "회원 생성을 위한 DTO")
public record MemberCreateRequestDto(
        @Schema(description = "회원 이름", example = "김춘식", required = true)
        @NotBlank(message = "이름은 필수입니다.")
        String name,

        @Schema(description = "회원 이메일", example = "exmaple@co.kr", required = true)
        @NotBlank(message = "이메일은 필수입니다.")
        @Email(message = "올바른 이메일 형식이 아닙니다.")
        String email,

        @Schema(description = "전화번호", example = "010-3434-6863", required = true)
